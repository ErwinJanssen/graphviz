...
static char *myfgets(char * ubuf, int n, FILE * fp)
{
	...
	if (!n)
	{ /* a call with n==0 (from aglexinit) resets */
		ubuf[0] = '\0';
		pos = len = 0;
		return NULL;
	}
...
graph_t *gvNextInputGraph(GVC_t *gvc)
{
	...
	while (!g)
	{
		if (!fp)
		{
			...
			{
				while ((fn = gvc->input_filenames[fidx++]) &&!(fp = fopen(fn,
						"r")))
				{
...
void graph_init(graph_t * g, boolean use_rankdir)
{
	...
	{ "local", "global", "none", NULL };
	static int rankcode[] =
	{ LOCAL, GLOBAL, NOCLUST, LOCAL };
	static char *fontnamenames[] =
	{ "gd", "ps", "svg", NULL };
	static int fontnamecodes[] =
	{ NATIVEFONTS, PSFONTS, SVGFONTS, -1 };
	int rankdir;
	...
	GD_drawing(g)->quantum = late_double(g, agfindgraphattr(g, "quantum"), 0.0,
			0.0);
	...
	if ((p = agget(g, "id")) && *p)GD_drawing(g)->id = strdup_and_subst_obj(p, g);
}
